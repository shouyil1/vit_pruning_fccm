...
from nn_pruning.modules.training_patcher_generic import (
    LinearModelPatcher,
    PatcherContext,
...
from model_variants_blockprune import struct_from_config, ModelStructureNotFound
from inference_patchers_blockprune import DeiTHeadsPruner
...
##################### NOTE ##############################################################
# Classes defined here should work for both DeiT and DeiT with tokens being dropped #####
#########################################################################################



@dataclass
class DeiTSparseTrainingArguments:
    """
...
class DeiTModelPatchingCoordinator:
    ...
    def __init__(self, sparse_args, device, cache_dir, model_name_or_path, logit_names, teacher_constructor):
        ...
        ############### NOTE ########################################
        # config will be DeiTConfig, generically 
        # this is okay even if used for DeiT with token drop
        # the config DeiTConfigDropTokens maps to DeiTStructure
        # same as DeiTConfig
        # this means we don't need to add anything else here
        # also, this is the only place where config is used
        #############################################################
    ...
    def create_teacher(self):
        ...
        if sparse_args.distil_teacher_name_or_path is not None:
            ...
            print("NOTE: Teacher model for distillation constructed from : \n", sparse_args.distil_teacher_name_or_path)
...
class DeiTSparseTrainer:
    def __init__(self, sparse_args):
        self.sparse_args = sparse_args
        self.loss_counter = 0
        self.metrics = defaultdict(float)

    def set_patch_coordinator(self, patch_coordinator: DeiTModelPatchingCoordinator):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        self.metrics["ce_loss"] += float(loss.mean())                                       # needed, need to understand precisely why         
