from deit_blockprune_run_params import (
    teacher_constructor,
    output_directory, 
...
if dataset_to_utilize_for_experiment == "cifar-10":
    ...
    train_ds, test_ds = load_dataset("imagenet-1k", split=['train[:1%]', 'validation[:1%]'])   # Validation is TEST
    
    splitter_to_train_val = train_ds.train_test_split(test_size=0.1)    # 25k images from train as part of val            
...
processor = DeiTImageProcessor.from_pretrained("facebook/deit-base-distilled-patch16-224")          # note that image processor for DeiT and DeiT drop tokens don't have any difference; drop tokens context is not needed for the processor
...
if force_load_classifier_parameters_for_experiments is not None:
    if (force_load_classifier_parameters_for_experiments) and (dataset_to_utilize_for_experiment == "imagenet-1k") and (model_to_utilize_for_experiment == 'deit' or model_to_utilize_for_experiment == 'deit_drop_tokens_variant_i'):
        ...
        _trash_model = DeiTForImageClassificationWithTeacher.from_pretrained("facebook/deit-base-distilled-patch16-224",
...
arg_set_normal['MODEL_NAME_FOR_EXPERIMENT'] = model_to_utilize_for_experiment
