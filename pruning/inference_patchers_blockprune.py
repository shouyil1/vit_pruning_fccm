...
from transformers import BertConfig, DeiTConfig

from deit_token_drop_variant_i import DeiTConfigDropTokens as DeiTConfigDropTokens_Variant_I

from nn_pruning.modules.model_patcher_generic import ModelPatcher

from model_variants_blockprune import struct_from_config, count_num_heads
...
###########################################################################################
###### DP: NOTE DeiTHeadsPruner prunes both DeiT and DeiT with drop tokens (Variant I) ####
###########################################################################################

class DeiTHeadsPruner:
    def __init__(self, model):
        self.model = model
        self.model_structure = struct_from_config(model.config_class)
        self.attention_head_size = self.model_structure.NAME_CONFIG["attention_head_size"]
    ...
    def run(self):
        model = self.model
        to_prune, heads_count = self.get_pruned_heads()
        if (isinstance(self.model.config, DeiTConfig) or isinstance(self.model.config, DeiTConfigDropTokens_Variant_I)):            # use the same head pruner for DeiTConfig and DeiTConfigDropTokens_Variant_I models since their structure is identical
            model.prune_heads(to_prune)
        return sum([len(p) for p in to_prune.values()]), heads_count
...
#############################################################################################
######## NOTE: This patcher is specifically for DeiT/DeiT Drop Token variants       #########
######## which can be extended, the inherent context does not clarify this however! #########
#############################################################################################
class InferenceModelPatcherDeiT(ModelPatcher):
    ...
    '''
    DP: Commenting this segment out; causes undefined import which cannot be resolved due to CUDA runtime conflict, not relevant since not used
...
def optimize_model_deit(model, mode, clone=True):
    ...
    mp = InferenceModelPatcherDeiT(prune_heads=True, mode=mode)
